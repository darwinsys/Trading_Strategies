...
'''
def atr_ratio(df, shortLen=10, longLen=20) :
    df_atr_short = atr(df, shortLen)
    df_atr_long = atr(df, longLen)
    v_atr_ratio = df_atr_short.values / df_atr_long.values

    return pd.DataFrame(data=v_atr_ratio, index=df.index,
                        columns=['atr_ratio_{m}_{n}'.format(m=shortLen, n=longLen)])
...
class CycleIndicator():
    ...
    and data[t+1]. CAUTION: the peaks and valleys at time t, can onlyh be calculated
